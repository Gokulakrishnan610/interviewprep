...
class BeyondPresenceService:
    def __init__(self):
        self.api_key = os.getenv("BEYOND_API_KEY")
        self.base_url = "https://api.bey.dev/v1"
        self.avatar_id = os.getenv("AVATAR_ID")
        
        if not self.api_key:
            raise ValueError("BEYOND_API_KEY not found in environment variables")
        if not self.avatar_id:
            raise ValueError("AVATAR_ID not found in environment variables")

    async def start_avatar_call(self, livekit_url: str, livekit_token: str, language: str = "english") -> dict:
        """Start an avatar call using Beyond Presence API"""
        ...
        async with aiohttp.ClientSession() as session:
            async with session.post(url, headers=headers, json=data) as response:
                if response.status == 200:
                    ...
                    raise Exception(f"Beyond Presence API error: {error_text}")

    async def end_avatar_call(self, call_id: str) -> dict:
        """End an avatar call"""
    ...
    async def get_avatar_status(self, call_id: str) -> dict:
        """Get avatar call status"""
        url = f"{self.base_url}/calls/{call_id}"
