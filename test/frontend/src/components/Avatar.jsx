...
import React, { useEffect, useRef, useState } from 'react'
import { Room, LiveKitRoom } from 'livekit-client'
import { createAgentToken } from '../api/livekit'

const Avatar = ({ roomName, userName, interviewPosition }) => {
  ...
  useEffect(() => {
    const initializeRoom = async () => {
      try {
        const tokenData = await createAgentToken(roomName, `agent-${Date.now()}`)
  ...
  const handleConnected = async (room) => {
    ...
    // Start Beyond Presence call
    try {
      const response = await fetch('/api/beyond/start-call', {
        method: 'POST',
        headers: {
      ...
      if (!response.ok) {
        throw new Error('Failed to start avatar call')
      }
      
      console.log('Avatar call started successfully')
    } catch (error) {
      console.error('Error starting avatar call:', error)
    }
  }
    ...
    <div className="avatar-container">
      ...
      >
        <div className="video-container">
          {/* LiveKit will automatically render video tracks */}
        </div>
